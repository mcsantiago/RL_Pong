...
def preprocess_frame(state):
    ...
    state = rgb2gray(state)     # Grayscale
    state = state[::2, ::2]      # Downsample by 2
    # state[state < 100] = 1      # To binary
    # state[state >= 100] = 0
...
if __name__ == "__main__":
    ...
    while time < n_frames:
        ...
        while not done:
            ...
            if state_reward > 0:
                player_score += state_reward
            else:
                enemy_score -= state_reward

            while next_state.shape[3] < k and not done:
                next_frame, reward, done, _ = env.step(action)
                next_state = np.append(
                    next_state, preprocess_frame(next_frame), axis=3)
                if reward > 0:
                    player_score += reward
                else:
                    enemy_score -= reward
